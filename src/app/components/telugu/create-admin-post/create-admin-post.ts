...
export class CreateAdminPost {
  ...
  async onFileSelected(event: Event, type: 'image' | 'video' | 'audio'): Promise<void> {
    ...
    if (type === 'video' || type === 'audio') {
      try {
        const duration = await this.getMediaDuration(file, type);
        if (duration > MAX_DURATION_SECONDS) {
          this.showError(`${type === 'video' ? 'Video' : 'Audio'} length cannot exceed 2 minutes.`);
          input.value = '';
          return;
        }
      } catch (err) {
        this.showError(`Could not load ${type} metadata. Please try another file.`);
        input.value = '';
        return;
      }
    }
  ...
  async publishPost(): Promise<void> {
    ...
    try {
      ...
      const postResponse: any = await firstValueFrom(this.service.postsAdmin(postData));
  ...
  async saveEdit(post: any): Promise<void> {
    ...
    try {
      ...
      const res: any = await firstValueFrom(this.service.updateAdminPost(post._id, updatePayload));
